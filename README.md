...
キーや `.env` をコミットしないことをコミット前に確認してください。CI はオープンソース版 Gitleaks の CLI コンテナで現在のファイルだけでなくコミット履歴も検査するため、checkout の履歴を省略しません。組織リポジトリでライセンスを要求する GitHub Action ラッパーは使用しません。検査でキーを検出した場合は、まず該当キーを**直ちに失効**させ、必要に応じて新しいキーへ交換します。その後に履歴から機密情報を除去し、CI が再び成功することを確認します。履歴の書き換えだけを失効の代わりにしてはいけません。
